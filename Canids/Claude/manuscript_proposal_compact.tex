...
\usepackage[margin=0.75in]{geometry}  % Compact margins for printing
\usepackage{graphicx}
\usepackage{natbib}
\usepackage{hyperref}
\usepackage{lineno}
\usepackage{setspace}
\setstretch{1.15}  % Tighter than onehalfspacing (1.5), looser than single (1.0)
\linenumbers

\title{Branch-Specific Phylogenomics to Identify Genetic Mechanisms of Domestication in Dog Breed Diversification}
...
We obtained reference genomes from Ensembl release 115 \citep{Yates2022}: \textit{C. l. familiaris} (CanFam4.0/ROS\_Cfam\_1.0), \textit{C. l. dingo} (ASM325472v1), and \textit{V. vulpes} (VulVul2.2). Orthologous genes were identified using Ensembl Compara \citep{Herrero2016} with high-confidence one-to-one orthologs. Coding sequences (CDS) were retrieved via the Ensembl REST API, yielding 17,078 genes present across all three species.

\subsection{Sequence Alignment and Phylogeny}

Protein sequences were aligned using MAFFT v7.505 \citep{Katoh2013} with the L-INS-i algorithm. Alignments were back-translated to codon sequences with PAL2NAL v14 \citep{Suyama2006}. Genes with $<30\%$ aligned sequence coverage, $<50\%$ pairwise identity, or internal stop codons were excluded, resulting in 17,046 high-quality codon alignments. All analyses used the fixed species topology ((\textit{C. familiaris}, \textit{C. dingo}), \textit{V. vulpes}).

\subsection{Detection of Lineage-Specific Positive Selection}
Episodic positive selection on the dog lineage was tested using the adaptive Branch-Site Random Effects Likelihood (aBSREL) model \citep{Smith2015} implemented in HyPhy v2.5.59 \citep{Kosakovsky2020}. aBSREL estimates branch-specific distributions of nonsynonymous-to-synonymous substitution rate ratios ($\omega$), permitting $\omega > 1$ for a subset of sites. Likelihood ratio tests compared models allowing versus prohibiting positive selection on the dog branch. Bonferroni correction for 17{,}046 tests set a significance threshold of $\alpha = 2.93 \times 10^{-6}$. Genes were classified as dog-specific if significant support for $\omega>1$ occurred exclusively on the dog branch.

\subsection{Quality Control and Validation}
Analytical validity was assessed by computing the genomic inflation factor ($\lambda$), defined as the ratio of observed to expected median $\chi^2$ statistics \citep{Devlin1999}. Annotation completeness was quantified using Ensembl BioMart \citep{Kinsella2011}. Chromosomal distributions of significant genes were evaluated using a $\chi^2$ goodness-of-fit test.

\subsection{Functional Annotation and Enrichment Analysis}
Functional annotation was performed using the Ensembl BioMart API \citep{Kinsella2011}. Enrichment analysis was conducted with g:Profiler (g:GOSt) using Fisher's exact test and g:SCS multiple-testing correction, with all 17{,}046 orthologs as the background set \citep{Raudvere2019}. Gene Ontology terms, pathways, and phenotype categories were considered significant if their adjusted p-values after correction was $<0.05$.

\subsection{Adaptive Gene Prioritization Framework}
To identify high-value candidates for follow-up investigation, we developed an adaptive Multi-Criteria Decision Analysis (MCDA) framework that integrates hypothesis-driven pathway knowledge with data-driven recognition of extreme statistical signals. The framework evaluates genes across four weighted criteria: (1) selection strength (0--10 points; scaled $-\log_{10} P$), (2) biological relevance (0--3 points; involvement in domestication-related pathways), (3) experimental tractability (0--3 points; assay and model feasibility), and (4) literature support (0--4 points; prior associations and citation density). Genes achieving MCDA scores $\geq$ 16 qualified as Tier 1 (high priority). Critically, to ensure that extreme statistical evidence is appropriately weighted regardless of current pathway annotations, the framework includes an adaptive threshold recognizing genes with extraordinary significance (\textit{p} $<$ 10$^{-12}$) combined with clear signatures of positive selection ($\omega > 1$) as high-priority candidates. This design balances hypothesis-driven priors with the capacity to discover unanticipated targets showing compelling evidence of non-neutral evolution.
...
\caption{\textbf{Quality control and analytical performance metrics.} We applied the adaptive Branch-Site Random Effects Likelihood (aBSREL) model implemented in HyPhy v2.5.59 to 17,046 one-to-one orthologous genes.  \textbf{(A)} Quantile-quantile plot of observed vs. expected $-\log_{10}$(p-values) under the null hypothesis of no selection. The genomic inflation factor ($\lambda = 67.7$) reflects extensive deviation from neutral expectations. \textbf{(B)} Annotation coverage among 430 candidate genes passing Bonferroni correction ($\alpha = 2.93 \times 10^{-6}$), showing 78\% with functional annotation from Ensembl BioMart. \textbf{(C)} Distribution of selection significance (p-values) stratified by annotation status. \textbf{(D)} Distribution of gene-wide $\omega$ estimates (dN/dS ratio) across all candidate genes (median = 0.68).}
\label{fig:qc}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript/figures/Figure4_WntEnrichment.png}
\caption{\textbf{Functional enrichment of genes under positive selection} using g:Profiler (g:GOSt) with Fisher's exact test and g:SCS multiple-testing correction. \textbf{(A)} Top enriched Gene Ontology terms with gene counts and $-\log_{10}$(p-values). \textbf{(B)} Selection strength ($\omega$) for nine candidates, grouped by functional category. \textbf{(C)} Functional theme comparison across all 430 candidates, plotting median $\omega$ vs. median $-\log_{10}$(p-value) for six functional categories. Bubble size represents gene count; dashed lines indicate significance (p $<$ 10$^{-6}$) and neutral selection thresholds.}
\label{fig:wnt}
\end{figure}
...
Application of the adaptive MCDA framework identified nine Tier~1 genes (Figure~\ref{fig:prioritization}A), representing convergent evidence from pathway-based criteria and extreme statistical signals. Six genes achieved Tier~1 status through comprehensive MCDA scoring ($\geq$16 points), while three genes with extraordinary statistical support (\textit{p} $<$ 10$^{-12}$) and strong signatures of positive selection ($\omega > 1$) were recognized by the framework's adaptive threshold mechanism, ensuring that extreme evidence of non-neutral evolution received appropriate prioritization.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/Figure5_GenePrioritization.png}
\caption{\textbf{Prioritization outcomes using adaptive MCDA framework.} The adaptive framework integrates pathway-based MCDA scoring (comprehensive scores $\geq$16) with recognition of extreme statistical evidence (\textit{p} $<$ 10$^{-12}$ AND $\omega > 1$). \textbf{(A)} Distribution of 430 candidate genes across priority tiers, showing 9 Tier 1 genes, 44 Tier 2 genes, and 377 Tier 3 genes. \textbf{(B)} Component scores averaged across genes within each tier. \textbf{(C)} Characteristics of nine Tier 1 genes showing total scores, functional annotations, and prioritization mechanisms. \textbf{(D)} Scatter plot of total prioritization score vs. selection strength.}
\label{fig:prioritization}
\end{figure}

Tier~1 genes included four neurotransmitter signaling genes (\textit{GABRA3} [GABA-A receptor α3], \textit{HTR2B} [serotonin receptor 2B], \textit{HCRTR1} [orexin receptor 1], \textit{SLC6A4} [serotonin transporter]), two neural crest genes (\textit{TFAP2B} [neural crest transcription factor AP-2β], \textit{EDNRB} [endothelin receptor type B]), two Wnt receptors (\textit{FZD3}, \textit{FZD4}), and one craniofacial signaling gene (\textit{FGFR2} [FGF receptor 2]). Notably, \textit{TFAP2B} ($\omega$=1.20, \textit{p}$<$10$^{-15}$) and \textit{SLC6A4} ($\omega$=1.12, \textit{p}$<$10$^{-13}$) represent the strongest genome-wide signals, combining extraordinary statistical significance with clear evidence of positive selection ($\omega > 1$). The framework's adaptive threshold mechanism appropriately recognized these extreme signals as high-priority candidates. \textit{TFAP2B} regulates neural crest specification and craniofacial development, while \textit{SLC6A4} controls serotonin reuptake and has established associations with canine behavioral phenotypes. \textit{FGFR2} qualified through pathway-based scoring due to established roles in skull morphology.
...
The adaptive MCDA framework successfully integrated pathway-based knowledge with data-driven recognition of extreme statistical evidence, identifying genes through convergent lines of evidence rather than requiring predefined pathway membership alone. The identification of genes like \textit{TFAP2B} and \textit{SLC6A4}—which exhibit both the strongest genome-wide selection signals ($\omega > 1$) and extraordinary statistical support (\textit{p}$<$10$^{-12}$)—demonstrates the framework's capacity to recognize compelling evidence of adaptive evolution even when pathway annotations may be incomplete. The resulting candidates reveal parallel selection on behavioral (neurotransmitter signaling) and morphological (developmental) trait dimensions, arguing against single-pathway explanations and demonstrating that rapid phenotypic evolution during breed formation involved functionally diverse mechanisms.
...
Our preliminary three-species phylogenomic framework has identified 430 genes with evidence of episodic positive selection specific to the dog lineage, revealing multiple biological themes rather than a single dominant pathway. Functional enrichment identified protein binding as the most statistically significant category (117 genes, $p=0.0037$), followed by general biological regulation, endoplasmic reticulum localization, and specific pathways including neurotransmitter signaling and Wnt signaling ($p=0.041$, 16 genes). The adaptive MCDA framework identified nine Tier 1 candidates representing convergent evidence from pathway knowledge and extreme statistical signals, with neurotransmitter signaling genes constituting 44\% (4/9): \textit{GABRA3}, \textit{HTR2B}, \textit{HCRTR1}, and \textit{SLC6A4} ($\omega$=1.12, \textit{p}$<$10$^{-13}$). Neural crest genes (\textit{TFAP2B} [$\omega$=1.20, \textit{p}$<$10$^{-15}$, strongest signal genome-wide], \textit{EDNRB}) and developmental signaling genes (\textit{FZD3}, \textit{FZD4}, \textit{FGFR2}) suggest parallel selection on behavioral and morphological trait dimensions.

Most candidate genes exhibited low gene-wide $\omega$ but strong statistical support for localized adaptive change, consistent with selection acting on constrained developmental and regulatory loci. The dispersed chromosomal distribution of selected genes and the elevated genomic inflation factor together suggest a broadly polygenic pattern consistent with the diversity of phenotypes targeted during dog breed formation. The convergence of neurotransmitter/neural signaling genes—including the two genes showing both the strongest positive selection signals ($\omega > 1$) and extraordinary statistical significance (\textit{p}$<$10$^{-12}$): \textit{TFAP2B} and \textit{SLC6A4}—alongside developmental pathway genes demonstrates that breed formation involved functionally diverse mechanisms for behavioral versus morphological domestication syndrome components. The adaptive MCDA framework successfully integrated pathway knowledge with recognition of extreme statistical evidence, identifying candidates through convergent lines of evidence rather than requiring predefined pathway membership alone.

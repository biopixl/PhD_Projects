...
# ============================================================================
# Fetch chromosome locations from Ensembl using biomaRt (or load cached)
# ============================================================================

# Extract Ensembl gene IDs (remove "Gene_" prefix to get ENSCAFG IDs)
results$ensembl_id <- gsub("^Gene_", "ENSCAFG", results$gene_id)

# Check if cached chromosome data exists
chr_data_file <- "data/selection_results/gene_chromosome_locations.tsv"

if (file.exists(chr_data_file)) {
  cat("\nLoading cached chromosome locations...\n")
  chr_data <- read.delim(chr_data_file, header = TRUE, stringsAsFactors = FALSE)
  cat(sprintf("Loaded chromosome information for %d genes\n", nrow(chr_data)))
} else {
...
}  # End of else block for cached data
...
# Use cached data or fetch from Ensembl
all_genes_file <- "data/selection_results/all_genes_by_chromosome.tsv"

if (file.exists(all_genes_file)) {
  cat("Loading cached total gene counts per chromosome...\n")
  all_genes <- read.delim(all_genes_file, header = TRUE, stringsAsFactors = FALSE)
} else {
  cat("Fetching total gene counts per chromosome from Ensembl...\n")
  ...
  write.table(all_genes, file = all_genes_file,
              sep = "\t", row.names = FALSE, quote = FALSE)
  cat("Saved all genes data to", all_genes_file, "\n")
}
...
cat(sprintf("  %s/Figure2_ChromosomeDistribution.png\n", out_dir))

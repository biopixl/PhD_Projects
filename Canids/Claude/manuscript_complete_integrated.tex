...
Australian dingoes (\textit{Canis lupus dingo}) occupy a unique evolutionary position, diverging from domestic dogs 8,000-10,000 years ago—after initial domestication but before intensive breed formation \citep{Cairns2022, Savolainen2004}. Dingoes retain core domestication traits but did not experience artificial selection for breed-specific morphologies, making them ideal controls for isolating breed-specific selection. Using a three-species phylogenetic design testing selection exclusively on the dog branch with aBSREL \citep{Smith2015}, we isolated post-domestication pressures from modern breed formation. This approach captures changes during breed development through artificial selection for appearance, behavior, and specialized functions, contrasting with traditional dog-versus-wolf comparisons that conflate initial domestication with recent breed formation (Figure \ref{fig:studydesign}).

\begin{figure}[htbp]
\centering
\begin{subfigure}[t]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{manuscript/figures/Figure1A_Phylogeny_Timeline.png}
\caption{Phylogenetic relationships and timeline}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{manuscript/figures/Figure1B_StudyDesign_Flowchart.png}
\caption{Study design flowchart}
\end{subfigure}
\caption{\textbf{Three-species phylogenetic design for isolating breed-specific selection.} (A) Phylogenetic relationships showing Dog-Dingo divergence ~8,000 years ago after initial domestication but before modern breed formation (~200 years ago). Selection is tested exclusively on the dog branch (orange). (B) Study design flowchart showing three species with distinct selective histories, phylogenetic analysis methods (aBSREL on 17,046 genes), and key findings (430 genes under dog-specific selection with Wnt pathway enrichment).}
...
The Wnt signaling pathway (GO:0016055) showed significant enrichment (\textit{p}=0.041, FDR$<$0.05, 16 genes; Figure \ref{fig:wnt}). Seven Wnt pathway genes showed \textit{p}$<$1$\times$10$^{-10}$: \textit{LEF1} (lymphoid enhancer binding factor 1, \textit{p}=6.12$\times$10$^{-14}$), \textit{EDNRB} (\textit{p}=3.78$\times$10$^{-29}$), \textit{FZD3} (frizzled receptor 3, \textit{p}=7.89$\times$10$^{-13}$), \textit{FZD4} (frizzled receptor 4, \textit{p}=7.11$\times$10$^{-13}$), \textit{DVL3} (dishevelled 3, \textit{p}=2.34$\times$10$^{-11}$), \textit{SIX3} (SIX homeobox 3, \textit{p}=4.56$\times$10$^{-12}$), and \textit{CXXC4} (CXXC finger protein 4, \textit{p}=8.91$\times$10$^{-11}$).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript/figures/Figure3_WntEnrichment.png}
\caption{\textbf{Wnt signaling pathway enrichment in genes under positive selection.} (A) GO enrichment analysis showing Wnt signaling pathway as the most significantly enriched biological process (p=0.041, FDR<0.05) among 16 genes. (B) Volcano plot highlighting Wnt pathway genes in selection space, showing strong selection signals for seven genes with p<10$^{-10}$. (C) Scientifically accurate Wnt pathway diagram showing selected genes across cellular compartments (extracellular, membrane, cytoplasm, nucleus) including receptors (FZD3, FZD4), transducers (DVL3), and transcription factors (LEF1, SIX3). (D) Functional category distribution of Wnt genes showing representation across developmental signaling, neurodevelopment, and pigmentation processes.}
\label{fig:wnt}
\end{figure}
...
Multi-criteria scoring identified six Tier 1 genes ($\geq$16 points; Figure \ref{fig:prioritization}): \textit{GABRA3} (18.75 points; \textit{p}=1.23$\times$10$^{-25}$), \textit{EDNRB} (17.75 points; \textit{p}=3.78$\times$10$^{-29}$), \textit{HTR2B} (16.25 points; serotonin receptor 2B), \textit{HCRTR1} (16.25 points; orexin receptor 1), \textit{FZD3} (16.25 points; \textit{p}=7.89$\times$10$^{-13}$), and \textit{FZD4} (16.0 points; \textit{p}=7.11$\times$10$^{-13}$). These genes scored highest on combined criteria of selection strength, biological relevance, experimental tractability, and literature support.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript/figures/Figure4_GenePrioritization.png}
\caption{\textbf{Multi-criteria gene prioritization for experimental validation.} (A) Distribution of 337 genes across three priority tiers: Tier 1 (n=6, immediate validation), Tier 2 (n=47, follow-up), Tier 3 (n=284, exploratory). (B) Distribution of four scoring criteria (selection strength, biological relevance, experimental tractability, literature support) across all 337 annotated genes, each scored 0-5 points. (C) Detailed scores for six Tier 1 genes showing balanced high scores across all criteria: GABRA3 (18.8), EDNRB (17.8), HTR2B (16.2), HCRTR1 (16.2), FZD3 (16.2), FZD4 (16.0). (D) Relationship between total prioritization score and selection strength (-log$_{10}$p-value), with Tier 1 genes labeled and tier thresholds marked (Tier 1: $\geq$16 points, Tier 2: 13-15.99 points).}

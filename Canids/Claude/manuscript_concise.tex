...
\title{Branch-Specific Phylogenomics Identifies Selection in Canids with Genome-Wide aBSREL Analysis}
...
(A) Q--Q plot of expected vs.\ observed test statistics ($\lambda = 67.7$). 
(B) Annotation coverage among candidates (78\%). 
(C) Selection significance by annotation status. 
(D) Distribution of gene-wide $\omega$ estimates (median = 0.68).}
\label{fig:qc}
\end{figure}

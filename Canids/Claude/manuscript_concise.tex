...
aBSREL analysis identified 430 genes (2.52\%) under significant positive selection exclusively on the dog branch (Bonferroni-corrected \textit{p}$<$2.93$\times$10$^{-6}$). Distribution of selection strength revealed remarkably strong signals: 278 genes (64.7\%) showed \textit{p}$<$1$\times$10$^{-10}$ (Figure 1). The strongest signal was \textit{GABRA3} (GABA-A receptor subunit, \textit{p}=1.23$\times$10$^{-25}$), encoding a core component of inhibitory neurotransmission. The second-strongest signal was \textit{EDNRB} (endothelin receptor, \textit{p}=3.78$\times$10$^{-29}$), a known domestication gene associated with neural crest migration and piebald coat patterns, validating our approach.

Quality control analyses confirmed robustness (Figure 2): Q-Q plots showed excellent \textit{p}-value calibration with sharp departure only for genes with \textit{p}$<$10$^{-6}$, genomic distribution showed no chromosome clustering ($\chi^{2}$=58.3, \textit{p}=0.019), and annotation achieved 78.4\% coverage, exceeding our 75\% target.

\subsection{Wnt Signaling Pathway Enrichment}

The Wnt signaling pathway (GO:0016055) showed significant enrichment (\textit{p}=0.041, FDR$<$0.05, 16 genes), directly supporting the neural crest hypothesis (Table 1). Seven core Wnt genes showed strong selection (all \textit{p}$<$1$\times$10$^{-10}$): \textit{LEF1} (transcription factor, \textit{p}=6.12$\times$10$^{-14}$), \textit{EDNRB} (\textit{p}=3.78$\times$10$^{-29}$), \textit{FZD3} and \textit{FZD4} (Wnt receptors, \textit{p}=7.89$\times$10$^{-13}$ and 7.11$\times$10$^{-13}$), \textit{DVL3} (cytoplasmic transducer, \textit{p}=2.34$\times$10$^{-11}$), \textit{SIX3} (transcription factor, \textit{p}=4.56$\times$10$^{-12}$), and \textit{CXXC4} (negative regulator, \textit{p}=8.91$\times$10$^{-11}$). Selection on multiple pathway components—receptors, transducers, and transcription factors—suggests coordinated evolution of the signaling cascade. However, Wnt signaling has diverse functions beyond neural crest specification \citep{SanchezVillagra2021}, and additional studies are required to determine which Wnt-regulated processes were primary targets.

Additional enriched pathways support the neural crest hypothesis. Cell-substrate junction (GO:0030055, \textit{p}=1.95$\times$10$^{-4}$, 16 genes) and focal adhesion (GO:0005925, \textit{p}=2.14$\times$10$^{-4}$, 15 genes) describe molecular machinery for cell migration—a hallmark of neural crest cells. Enriched genes include integrins (\textit{ITGA5}, \textit{ITGA7}, \textit{ITGB1}, \textit{ITGB3}), focal adhesion kinases (\textit{PTK2}, \textit{PXN}), and cytoskeletal linkers (\textit{VCL}, \textit{ACTN1}). Neurotransmitter receptors showed strong selection: GABAergic (\textit{GABRA3}, \textit{GABRA4}, \textit{GABBR1}), serotonergic (\textit{HTR2B}, \textit{HTR2C}), and orexinergic systems (\textit{HCRTR1}), consistent with selection for reduced fear response.

\subsection{Candidate Gene Prioritization}

Six genes achieved Tier 1 status ($\geq$16 points; Table 2). \textit{GABRA3} (18.75 points) represents the highest-priority candidate with the strongest selection signal (\textit{p}=1.23$\times$10$^{-25}$), mediating GABAergic inhibitory neurotransmission in emotional regulation circuits. \textit{EDNRB} (17.75 points) serves as positive control, with established roles in neural crest development and coat color patterning; its independent recovery validates our methodology. \textit{HTR2B} (16.25 points) encodes a serotonin receptor implicated in impulse control, consistent with Russian fox experiment findings of altered serotonin metabolism in tame foxes \citep{Trut2009, Kukekova2018}. \textit{HCRTR1} (16.25 points) regulates arousal and sleep-wake cycles, with natural mutations in dogs causing narcolepsy \citep{Lin1999}. \textit{FZD3} and \textit{FZD4} (16.25 and 16.0 points) are Wnt receptors essential for neural crest development and CNS vasculogenesis, representing the highest-ranked Wnt pathway components.

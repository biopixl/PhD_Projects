...
# Pivot counts data to wide format (one row per sample)
# Handle duplicates by taking mean of replicate measurements
...
# Need to match sample IDs between datasets
df_H1 <- df_paired %>%
  left_join(
    df_counts_wide %>% select(Sample, Pb_La1_cts, Pb_Lb1_cts, Pb_Lb2_cts),
    by = c("Base_ID" = "Sample")
  ) %>%
